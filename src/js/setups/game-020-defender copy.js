...
const createButtonBaseCreatorBaby = (x, y, radius) => {
    ...
    const result = guiFollowCircle({
        ...
        onExitActive: (sender) => {
            buttonsBases.push(createButtonBase(result.getX(), result.getY()))
    ...
    result.handle = (grid, elapsed) => {
        ...
        timer += elapsed
        if (timer > 3) {
            // check if nearby any base existing
            let smallestdist = 100000;
            let smallestindex = -1
            buttonsBases.forEach((base, index) => {
                smallestdist = Math.min(smallestdist, getItemDistance(base, result))
                smallestindex = index

            })
            if (smallestdist < radius + buttonsBases[smallestindex].getRadius()) {
                // add baby size to base nearbiest
                buttonsBases[smallestindex].setRadius(buttonsBases[smallestindex].getRadius() + radius / 4)
                if (buttonsBases[smallestindex].getRadius() > 200) {
                    buttonsBases[smallestindex].setRadius(200)
                }
            } else {

                // create new base at position after time lapsed
                buttonsBases.push(createButtonBase(result.getX(), result.getY(), result.getRadius()))
            }

            removeItemFromArray(buttonsBabyBases, result)
        }
    }
    return result
}
...
var buttonsBases = [createButtonBase()];
const radius = 1920 / 2
function createEnemyTypeBoxed() {
    // 4 new spawns

}

function createEnemyTypeSlow() {
    // just slow 
}

function createEnemyTypeFast() {
    // just fast
}
...
function createEnemy() {
    ...
    const result = guiFillButton({
        label: '',
        posX: (1920 / 2) + Math.sin(randi * Math.PI * 2) * radius,
    ...
    let speedX = (buttonsBases[index].getX() - result.getX()) / radius
    let speedY = (buttonsBases[index].getY() - result.getY()) / radius
