...
var PhysicsPong = require('./PhysicsSorter').default;
...
PhysicsPong.callBackObstacleCreated = (enemy) => {
    paddles.push({
        ...
        paddle: createPaddle({
            ...
            radius: enemy.shapes[0].radius ? enemy.shapes[0].radius : enemy.shapes[0].width / 2
        })
    })
}
...
import { BALL_TYPES, OBSTACLE_TYPES } from './PhysicsSorter';
  ...
  `,
    ...
    handle: function (grid, elapsed) {
        ...
        if (gameState == 'start') {
            ...
            paddles.forEach((item) => {
                if (item.paddle.getIsCurrentlyHavingMoreThanZeroLaserPointersDetected()) {
