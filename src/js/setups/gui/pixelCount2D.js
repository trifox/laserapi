...
export default ({
  ...
  label3 = 'Pixel Counter 1d',
  label4 = 'Pixel Counter 1d',
  posX,
  posY,
  radius = 10,
  normalColor = '#008844',
  ...
  return {
    ...
    handle: function (grid) {
        ...
        for (var y = 0; y <= (radius * 2) / facty; y++) {
          ...
          if (
            moveToHelper_getGridPixel(
              ...
              y + Math.floor((currentY - radius) / facty)
            ) > 0
          ) {
            pixelCount++;
            meanX += x
      ...
      if (live) {
        ...
        ctx.fillRect(
          currentX - radius + meanX - 12.5,
          currentY - radius + meanY - 12.5,
          25, 25
        );
      }

      // make line rasterized        

      for (var i = 0; i < 11; i++) {
        drawLine(ctx,
          currentX - radius, currentY - radius + i * (radius * 2 / 10),

          currentX + radius, currentY - radius + i * (radius * 2 / 10), '#444444')
        drawLine(ctx,
          currentX - radius + i * (radius * 2 / 10), currentY - radius,

          currentX - radius + i * (radius * 2 / 10), currentY + radius, '#444444')
      }
      drawLine(ctx, currentX - radius - 10, currentY + radius, currentX + radius, currentY + radius, '#888888')
      drawLine(ctx, currentX - radius, currentY + radius + 10, currentX - radius, currentY - radius, '#888888')
