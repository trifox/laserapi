...
const spawnBall = function (world, { x, y, dirX, dirY, type = BALL_TYPES.NORMAL }) {
  ...
  const circleShape = new p2.Circle({ radius: type === BALL_TYPES.NORMAL ? ballRadius : ballRadius * 2 });
...
const spawnManager = (elapsed) => {
  ...
  if (spawnBurstCount < 0) {
    ...
    spawPosY = Math.random() * (1080 - 200) + 100
    spawPosX = Math.random() * (250) - 250
    spawPosDirY = Math.random() * (60) - 30
    spawPosDirX = Math.random() * (60) - 30
    spawAngleStart = 0
    spawAngleAdd = Math.random() * 25 - 12.5
    currentSpawnCount = 0 + waveCount // number of balls in this wave
    frequency = Math.random() + 0.01 // spawn wave item frequency
    spawnBurstCount = 5 + currentSpawnCount // every 20 seconds new wave
    spawnedBallsCount = 0
    spawnBallSpeed = Math.random() * 200 + 100
  ...
  if (spawnCount < 0 && spawnedBallsCount < currentSpawnCount) {
    ...
    balls.push(spawnBall(world, { x: spawPosX, y: spawPosY, dirX: velocityX, dirY: velocityY, type: spawnType }))
  ...
  world.on('beginContact', (data) => {
    ...
    if (data.bodyA.laserType === BALL_TYPES.RAMMBOCK && data.bodyB.laserType === 'obstacle') {
      ...
      if (data.bodyB.shapes[0].radius) {
        ...
        data.bodyB.shapes[0].width /= 2
        data.bodyB.shapes[0].height /= 2
      ...
      if (data.bodyA.shapes[0].radius) {
        ...
        data.bodyA.shapes[0].width /= 2
        data.bodyA.shapes[0].height /= 2
...
function renderGamePlayfield(ctx) {
  ...
  const drawBall = ((circleBody, ballRadius, type = BALL_TYPES.NORMAL) => {
    ...
    drawNgon({
      ctx,
      numberOfSides: type === BALL_TYPES.RAMMBOCK ? 4 : type === BALL_TYPES.EXPLODE ? 4 : 16,
  ...
  ctx.textAlign = 'left';
  ctx.fillText(
    `  Health: ${maxLife - escapedBalls} Wave: ${waveCount} Next Wave: ${Math.ceil(spawnBurstCount)}s`,
    100,
