...
const handler = function (laserGrid) {
  ...
  if (!kernelBuffer) {
    kernelBuffer = gpu
      .createKernel(function (grid, buffer, elapsed, gridSizeX) {
        const pos = this.thread.x;
        var value = 0;
        for (var i = 0; i < 5; i++) {
          for (var j = 0; j < 5; j++) {
            value += grid[pos + i + j * gridSizeX];
          }
        }
        if (value > 0) {
          return Math.min(buffer[pos] + 4 * elapsed, 1);
        } else {
    ...
    kernelRender = gpu
      .createKernel(function (
        ...
        if (value > dist) {
          ...
          this.color(0 * rgbs[0], value * rgbs[1], value * rgbs[2], value);
        } else {
          this.color(0, 0, 0, 0);
        }
      })
  ...
  buffer = kernelBuffer(laserGrid, buffer, elapsed, laserConfig.gridResolution);
...
export default {
  name: 'Laser-Montagsmaler',
  description: `
  Malen mit Laserpointer, 
  dein Laserpointer erzeugt eine Spur.

  Das Bild verschwindet langsam, 
  und kann uebermalt werden.
  `,
  handle: handler,
  init: () => {},
};
